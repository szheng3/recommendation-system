...
class QNetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain, name='DQNetwork'):
        ...
        self.discrete_cql = DiscreteCQL(q_func_factory=QFunctionFactory(),
                                scaler=None)
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            cql_loss=0
            if self.use_CQL:
                cql_loss = self.discrete_cql.compute_loss(self.output1, self.actions, target_q=self.targetQs_)
                cql_loss_negative = self.discrete_cql.compute_loss(self.output1, self.actions, self.negative_actions, self.CQL_alpha)
                cql_loss_positive = self.discrete_cql.compute_loss(self.output1, self.actions, self.positive_actions, self.CQL_alpha)
                

            ### Incorporating CWL into loss1
            if self.use_CQL:
                self.loss1 = tf.reduce_mean(input_tensor=qloss_positive+cql_loss_positive + qloss_negative + cql_loss_negative + ce_loss_pre) 
